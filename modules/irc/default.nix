...
in {
    ...
    in
      lib.nameValuePair "pounce-${network}" {
        ...
        serviceConfig =
          ...
          // {
            ExecStart =
              "${pkgs.pounce}/bin/pounce ${cfgFile} \${CREDENTIALS_DIRECTORY}/join "
    ...
    in
      lib.nameValuePair "pounce-palaver-${network}" {
        ...
        serviceConfig =
          ...
          // {
            ExecStart = "${pkgs.pounce}/bin/pounce-palaver -d ${home}/${network}.sqlite -p ${toString attrs.local-port} ${localHost}";
            ReadWritePaths = home;
            Restart = "on-failure";
    ...
    in
      lib.nameValuePair "litterbox-${network}" {
        ...
        serviceConfig =
          ...
          // {
            ExecStart = "${pkgs.litterbox}/bin/litterbox ${cfgFile}";
            ReadWritePaths = home;
            Restart = "on-failure";
  ...
  in
    ...
    // {
      litterbox-init = {
        serviceConfig = {
          Type = "oneshot";
          ExecStart = "${pkgs.litterbox}/bin/litterbox -i -d ${litterbox}";
  ...
  users.users =
    ...
    // {
      iliana.packages = [pkgs.litterbox pkgs.weechat];
    };
